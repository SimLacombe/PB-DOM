...
chains <- list(wolfe = c(236508, 344297))
...
france  <- filter(read_sf("wolfe_case_study/data/countries_shp/"), NAME == "France")
X       <- readRDS("wolfe_case_study/data/envcov_louvrier.rds")
effort  <- readRDS("wolfe_case_study/data/effort_louvrier.rds")
xy      <- select(readRDS("wolfe_case_study/data/coordcells_wolf.rds"), X, Y)
...
fit <- map(paste0("wolfe_case_study/out/CHAIN_", chains$wolfe, "/"), load_chain, T = T, N = N)
